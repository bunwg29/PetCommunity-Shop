...
export const index = async (req: Request, res: Response) => {
  const pagination = await Pagination(req, FoodOrderModel, {});
...
export const detail = async (req: Request, res: Response) => {
  ...
  const orderDetail = await FoodOrderModel.findOne({ _id: orderId });
...
export const confirmOrder = async (req: Request, res: Response) => {
  ...
  const order = await FoodOrderModel.updateOne(
    { _id: orderId },
    { status: 'Confirmed' }
  );
...
export const deleteOrder = async (req: Request, res: Response) => {
  ...
  const order = await FoodOrderModel.findByIdAndDelete(orderId);
