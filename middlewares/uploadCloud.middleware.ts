...
export const uploadMultipeTypeForm = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (files["images"] && files["images"].length > 0) {
      const uploadPromises = files["images"].map((file: Express.Multer.File) => streamUpload(file.buffer));
    ...
    res.status(500).json({ success: false, message: "Error uploading files to Cloudinary." });
  }
