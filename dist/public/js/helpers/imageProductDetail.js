...
additionalImages.forEach((item) => {
  item.addEventListener('click', () => {
    styleBorder(item);
    updateMainImage(
      item.getAttribute('image_position'),
...
arrowLeft.addEventListener('click', () => {
  let positionMainImage = parseInt(
    mainImage.getAttribute('image_position'),
    10
  );
  ...
  const newSrc =
    positionMainImage > 0
      ...
      : document
          .querySelector(`img[image_position="main_avt"]`)
          .getAttribute('src');
